...
import { useCreateFacilities, useUpdateFacilities } from "@/hooks/useFacilities";
...
type Props = { facility?: Facilities | null };

export default function AdminFacilitiesForm({ facility }: Props) {
  const router = useRouter();
  const [title, setTitle] = useState(facility?.title ?? "");
  const [slug, setSlug] = useState(facility?.slug ?? "");
  const [excerpt, setExcerpt] = useState(facility?.excerpt ?? "");
  const [body, setBody] = useState(facility?.body ?? "");
  const [published, setPublished] = useState(facility?.published ?? false);
  const [image, setImage] = useState(facility?.image ?? "");
  const [error, setError] = useState("");
  const submitAsPublishRef = useRef(true);

  const isEdit = !!facility?.id;
  const createMutation = useCreateFacilities();
  const updateMutation = useUpdateFacilities();
  ...
  async function handleSubmit(e: React.FormEvent, asPublished: boolean) {
    ...
    try {
      if (isEdit && facility) {
        ...
        router.push(`/admin/facilities/${created.id}`);
        router.refresh();
      }
    } catch (err) {
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <textarea
              ...
              placeholder="Short summary shown in facility listings…"
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <RichTextEditor
              ...
              placeholder="Write your facility content here… Use the toolbar for bold, italic, bullet points, headings and more."
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-xs text-gray-400">
              {isEdit ? 'Click Publish to make this facility live.' : 'Use Save Draft to keep it hidden or Publish to go live.'}
            </p>
          </div>
